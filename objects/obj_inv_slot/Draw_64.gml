	...
	if(sprite_exists(spr)) {
		...
		if(inv[# s_inventory.slot_cap,slot_y] > 1) {
			if(inv[# s_inventory.slot_id,slot_y] == "gold") {
				draw_text(x+20,y+20,string(inv[# s_inventory.slot_sum,slot_y]));
			}
			else {
				draw_text(x+20,y+20,string(inv[# s_inventory.slot_sum,slot_y]) + "/" + string(inv[# s_inventory.slot_cap,slot_y]));
			}
		}
	}
}